		...
		<> session_pid è valorizzato ?
			==> se si
				<> session_pid è uguale al pid trovato ?
					==> se si, uscire dal ciclo e lfseek(fp,0,SEEK_END)
...
*/
	...
	if ( strstr(log_line," connect from")!=NULL )
	{
		if ( get_pid(&log_line,&found_pid)==NULL )
			...
			if( add_pid_to_list(found_pid,log_line)==NULL )
			{
	...
	if ( strstr(log_line,session_id)!=NULL )
	{
		...
		{
			if ( is_pid_in_list(found_pid)==NULL )
			{
	...
	if ( strstr(log_line," disconnect from")!=NULL )
	{
		if ( get_pid(&log_line,&found_pid)==NULL )
			...
			if(session_pid!=NULL && strcmp(session_pid,found_pid)==0)
			{
				...
				strcpy(close_session_line,log_line);
				lfseek(fp,0,SEEK_END);
			}
			else
				if ( remove_pid_from_list(found_pid)==NULL )
				{
